...
const blog = defineCollection({
  ...
  schema: z.object({
    title: z.string(),
    // What the post is about, in a sentence, for search results and link
    // previews. The archive text itself stays untouched.
    description: z.string().optional(),
    // A shorter title for search results when the post's own title runs past
    // what a result can show. The post keeps its title on the page.
    seoTitle: z.string().optional(),
    // Language of the post. Most of the archive is English; a Swedish post says
    // so, and names the post it is a version of so the two point at each other.
