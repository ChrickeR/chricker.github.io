...
export default defineConfig({
  ...
  redirects: {
    ...
    '/services/chief-architect': '/services/#interim-chief-architect',
    '/services/engineering-manager': '/services/',
    '/services/interim-cio-cto': '/services/#interim-chief-architect',
    '/services/solution-architect': '/services/#solution-design',
    '/services/sub/advisory/advisory': '/services/#interim-chief-architect',
    '/services/sub/interim/interim': '/services/#interim-chief-architect',
    '/services/sub/leadership': '/services/#interim-chief-architect',
    '/services/sub/operating_models/operating_models': '/services/#capability-and-domain-design',
    '/services/sub/solutions': '/services/#solution-design',
    '/sv': '/tjanster/',
  ...
  integrations: [
    sitemap({
      // Blog posts carry their date in the URL, which makes it a lastmod that
      // is actually true. Other pages leave it out rather than claim every
      // build as a change; their structured data carries the real dates.
      serialize(item) {
        const m = item.url.match(/\/blog\/(\d{4})\/(\d{2})\/(\d{2})\//);
        if (m) item.lastmod = `${m[1]}-${m[2]}-${m[3]}`;
        return item;
      },
      ...
      // A standalone page in public/, so the integration cannot see it.
      customPages: ['https://troi.se/tools/cambium-metamodel.html'],
      filter: (page) =>
