...
var launchEditor = _interopDefault(require('launch-editor'));
...
class AdrSlug extends ValueObject {
  ...
  static createFromTitle(title, packageRef, date) {
    ...
    const localSlug = `${moment$1(date).format("YYYYMMDD")}-${slugifiedTitle}`;
    return new AdrSlug(packageRef ? `${packageRef.name}/${localSlug}` : localSlug);
  }
...
class Adr extends AggregateRoot {
  ...
  get publicationDate() {
    ...
    const date = moment.tz(`${dateStr} 23:59:59`, dateFormats.map(format => `${format} HH:mm:ss`), true, Adr.tz);
...
class AdrRepository {
  ...
  async getLastEditDateFromFilesystem(file) {
    const stat = await fs.promises.stat(file.path.absolutePath);
    return stat.mtime;
  }
  ...
  async findAllInPath(p, packageRef, filter) {
    const files = await fs.promises.readdir(p.absolutePath);
    return Promise.all(files.map(filename => {
      ...
      return fs.promises.readFile(adrFile.path.absolutePath, {
        encoding: "utf8"
      }).then(async markdown => {
        const baseAdrProps = {
          slug,
  ...
  generateAvailableSlug(title, packageRef) {
    ...
    do {
      ...
      i += 1;
    } while (fs__default.existsSync(path.join(adrFolderPath.absolutePath, filename)));
  ...
  async save(adr) {
    ...
    if (!file) {
      ...
      if (fs__default.existsSync(file.path.absolutePath)) {
    ...
    await fs.promises.writeFile(file.path.absolutePath, adr.body.getRawMarkdown(), {
      encoding: "utf-8"
    });
  }
...
class AdrTemplateRepository {
  ...
  async find(packageRef) {
    ...
    const markdown = await fs.promises.readFile(templatePath, {
      encoding: "utf8"
    });
    return new AdrTemplate({
      package: packageRef,
...
class PackageRepository {
  ...
  buildPackage(name, projectPath, adrFolder) {
    ...
    if (!fs__default.existsSync(pkg.path.absolutePath)) {
      throw new Log4brainsError("Package path does not exist", `${pkg.path.pathRelativeToCwd} (${pkg.ref.name})`);
    }

    if (!fs__default.existsSync(pkg.adrFolderPath.absolutePath)) {
...
function buildContainer(config, workdir = ".") {
  ...
  container.register({
    ...
    workdir: awilix.asValue(workdir),
    fileWatcher: awilix.asClass(FileWatcher).singleton()
  }); // Repositories

  Object.values(repositories).forEach(Repository => {
    container.register(lowerCaseFirstLetter(Repository.name), awilix.asClass(Repository).singleton());
  }); // Command handlers

  Object.values(adrCommandHandlers).forEach(Handler => {
    container.register(Handler.name, awilix.asClass(Handler).singleton());
  }); // Command bus

  container.register({
    commandBus: awilix.asFunction(() => {
...
function buildConfigFromWorkdir(workdir = ".") {
  ...
  try {
    const content = fs__default.readFileSync(configPath, "utf8");
...
function findWorkdirRecursive(cwd = ".") {
  ...
  if (fs__default.existsSync(path.join(cwdAbsolute, configFilename))) {
    return cwdAbsolute;
  }
...
exports.Log4brains = Log4brains;
exports.Log4brainsConfigNotFoundError = Log4brainsConfigNotFoundError;
exports.Log4brainsError = Log4brainsError;
