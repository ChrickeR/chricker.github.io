...
class NewCommand {
  ...
  async execute(opts, titleArg) {
    ...
    if (opts.quiet && !titleArg) {
      throw new core.Log4brainsError("<title> is required when using --quiet");
    }
    ...
    if (opts.from) {
      if (!fs__default.existsSync(opts.from)) {
        throw new core.Log4brainsError("The given file does not exist", opts.from);
      } // TODO: use streams


      await fs.promises.writeFile(adrDto.file.absolutePath, await fs.promises.readFile(opts.from, "utf-8"), "utf-8");
    }
...
function execWithErrorHandler(promise, appConsole) {
  promise.catch(err => {
    if (err instanceof core.Log4brainsError && err.name === "The template.md file does not exist") {
      appConsole.error(err);
      appConsole.printlnErr(`You can use this ${terminalLink("template", templateExampleUrl)} as an example`);
      throw new cliCommon.FailureExit();
    }
  });
  return promise;
}
